...
class Scanner
  def initialize(paths_filename, nmap_filename, target_file, savedURLs_filename, target_ips_range, scan_port_range, scan_all_ports, brute_force_mode, number_of_threads)
    ...
    # File with exploitable URLs saved from last webscanner run
    ...
    begin
      @webscannerdb = SQLite3::Database.new @outdb
      @webscannerdb.execute "CREATE TABLE IF NOT EXISTS VulnApps(AppName STRING, AppURL STRING, Exploit STRING, Username STRING, Password STRING)"
...
private
  ...
  def process_savedgoodURLs_file
    ...
    $logfile.info("--------------------------------------------------------")
    $logfile.info("<<<webscanner discovered following vulnerable applications>>>")
    $logfile.info("--------------------------------------------------------")
    $logfile.info("#{@info}")
    ...
    puts ""
    puts "--------------------------------------------------------"
    puts "<<<webscanner discovered following vulnerable applications>>>".green
    puts "--------------------------------------------------------"
        ...
        if resp
          ...
          when "200"
            ...
            if not resp.body.scan(/<form/i).empty? and not resp.body.scan(/login/i).empty?
              $logfile.info("[+] webscanner found #{appkey} at #{attack_url}. May require form based auth")
              puts "[+] webscanner found #{appkey} at #{attack_url}. May require form based auth".green
              ...
              $logfile.info("[+] webscanner found an unauthenticated instance of #{appkey} at #{attack_url}.")
              puts "[+] webscanner found an unauthenticated instance of #{appkey} at #{attack_url}.".green
              creds = ["None", "None"]
            end
            ...
            @webscannerdb.execute("INSERT INTO VulnApps (AppName, AppURL, Exploit, Username, Password) VALUES(?, ?, ?, ?, ?)", appkey, attack_url, exploit_path, creds[0], creds[1])
            #break
          ...
          when "403"
            ...
            if not resp.body.scan(/<form/i).empty? and not resp.body.scan(/login/i).empty?
              puts "[+] webscanner found #{appkey} at #{attack_url}. Says not authorized but may contain login page".green
              if @brute_force_mode == 'form' or @brute_force_mode == 'all'
          ...
          when "401"
            ...
            $logfile.info("[+] webscanner found #{appkey} at #{attack_url}. Requires HTTP basic auth")
            puts "[+] webscanner found #{appkey} at #{attack_url}. Requires HTTP basic auth".green
  ...
  def brute_force_basic_auth(url401,dcreds)
    ...
    if response and (response.code == "200" or response.code == "301")
      $logfile.info("[+] webscanner, found default login credentials for #{url401} - #{username}:#{password}\n")
      puts ("[+] webscanner, found default login credentials for #{url401} - #{username}:#{password}\n").green
...
if __FILE__ == $0
  ...
  OptionParser.new do |opts|
    opts.banner = "webscanner #{VERSION}"
  ...
  if not File.exists?(options.paths_file)
    puts "webscanner needs a YAML file of path signatures to function.".red
    #$logboth.info('webscanner needs a YAML file of path signatures to function.')
    exit
  end
